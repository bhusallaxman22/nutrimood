...
import { EnhancedChart } from "./components";
import { styles } from "./styles";
import type { MoodData, MoodTrendsState, TimeRange } from "./types";
import {
    animateChart,
    animateStats,
    getAverageMood,
...
const MoodTrendsScreen: React.FC = () => {
  const LineChart = useOptionalChart();
        ...
        if (gestureState.dx > 50) {
          ...
          handleTimeRangeSwipe(
            ...
            (fn: (prev: number) => number) =>
              setState((prev) => ({
                ...prev,
                interactionCount: fn(prev.interactionCount),
              }))
          );
  ...
  }, [fadeAnim, slideAnim, scaleAnim, pulseAnim, rotateAnim, floatingMoods]);
  ...
  useEffect(() => {
    const fetchMoodData = async () => {
      try {
        ...
        if (data.length > 0) {
          ...
          const daysToShow =
            state.selectedTimeRange === "7days"
              ? 7
              : state.selectedTimeRange === "30days"
              ? 30
              : data.length;
    ...
    fetchMoodData();
  }, [state.selectedTimeRange, statsAnimations, chartAnimations]); // Re-fetch when time range changes
  ...
  return (
    <GlassBackground
      variant="neutral"
      pattern={false}
      gradient={colors.gradients.neutralApp}
    >
        ...
        >
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                toggleInsights(
                  state.showInsights,
                  (show: boolean) =>
                    setState((prev) => ({ ...prev, showInsights: show })),
                  (fn: (prev: number) => number) =>
                    setState((prev) => ({
                      ...prev,
                      interactionCount: fn(prev.interactionCount),
          ...
          <View style={styles.timeRangeContainer}>
            ...
            <Text style={styles.swipeHint}>
              👆 Tap to select or swipe to change
            </Text>
          </View>
            ...
            <>
              ...
              >
                ...
                >
                  <TouchableOpacity
                    ...
                    activeOpacity={0.9}
                  >
                    <View style={[styles.enhancedStatCard,{backgroundColor:'#667eea'}]}>
                      <View style={styles.statIconContainer}>
                        <Text style={styles.statEmoji}>📊</Text>
                        <View style={styles.statIconGlow} />
                      </View>
                      <Text style={styles.enhancedStatValue}>
                        {state.recentMoods.length}
                      </Text>
                      <Text style={styles.enhancedStatLabel}>
                        Total Entries
                      </Text>
                        ...
                        <View
                          style={[
                            styles.statProgressFill,
                            {
                              width: `${Math.min(
                                100,
                                (state.recentMoods.length / 50) * 100
                              )}%`,
                            },
                          ]}
                        />
                ...
                >
                  <TouchableOpacity
                    ...
                    activeOpacity={0.9}
                  >
                    <View style={[styles.enhancedStatCard,{backgroundColor:'#ffecd2'}]}>
                      <View style={styles.statIconContainer}>
                        <Text style={styles.statEmoji}>🎯</Text>
                        <View style={styles.statIconGlow} />
                      </View>
                        ...
                        {[
                          ...
                          <View
                            ...
                            style={[
                              ...
                              {
                                backgroundColor:
                                  getAverageMood(state.recentMoods) === level
                                    ? "#8B4513"
                                    : "rgba(139, 69, 19, 0.3)",
                              },
                            ]}
                ...
                >
                  <TouchableOpacity
                    ...
                    activeOpacity={0.9}
                  >
                    <View style={[styles.enhancedStatCard,{backgroundColor: getMoodTrend(state.moodData)==='improving'? '#a8edea': getMoodTrend(state.moodData)==='declining'? '#ffecd2':'#d299c2'}]}>
                      <View style={styles.statIconContainer}>
                        <Animated.Text
                          style={[
                            styles.statEmoji,
                      ...
                      <Text
                        style={[
                          ...
                          {
                            color:
                              getMoodTrend(state.moodData) === "improving"
                                ? "#2D5A5A"
                                : getMoodTrend(state.moodData) === "declining"
                                ? "#8B4513"
                                : "#6B5B95",
                ...
                >
                  <TouchableOpacity
                    ...
                    activeOpacity={0.9}
                  >
                    <View style={[styles.enhancedStatCard,{backgroundColor: getStreakDays(state.recentMoods)>7? '#ff9a9e':'#ffecd2'}]}>
                      <View style={styles.statIconContainer}>
                        <Animated.Text
                          style={[
                            styles.statEmoji,
                      ...
                      <Text
                        style={[
                          ...
                          {
                            color:
                              getStreakDays(state.recentMoods) > 7
                                ? "#8B0066"
                                : "#8B4513",
                          },
                        ]}
                      ...
                      <View style={styles.streakVisualization}>
                        {Array.from({
                          length: Math.min(getStreakDays(state.recentMoods), 7),
                        }).map((_, index) => (
                          <Animated.View
                            key={index}
                            style={[
                      ...
                      >
                        {getStreakDays(state.recentMoods) === 0
                          ? "Start your streak!"
                          : getStreakDays(state.recentMoods) < 3
                          ? "Building momentum!"
                          : getStreakDays(state.recentMoods) < 7
                          ? "Great consistency!"
                          : "Amazing dedication!"}
                ...
                >
                  <GlassCard style={styles.frequentMoodCard} variant="elevated">
                    ...
                    <View style={styles.frequentMoodDisplay}>
                      <View style={[styles.frequentMoodGradient,{backgroundColor:'#6366f1'}]}>
              ...
              <GlassCard style={styles.breakdownCard} variant="elevated">
                ...
                <View style={styles.moodBreakdown}>
                  {Object.entries(state.moodStats).map(
                    ([mood, count], index) => (
                      <TouchableOpacity
                        key={mood}
                        style={[
                          ...
                          state.selectedMood === mood &&
                            styles.moodBreakdownItemSelected,
                        ]}
                        onPress={() =>
                          handleMoodSelect(
                            mood,
                        ...
                        activeOpacity={0.7}
                      >
                        <View style={[styles.moodBreakdownGradient,{backgroundColor:'#6366f1'}]}>
                          <View style={styles.moodBreakdownLeft}>
                            <Text style={styles.moodBreakdownEmoji}>
                              {getMoodEmoji(mood)}
                            </Text>
                            <View>
                              <Text style={styles.moodBreakdownMood}>
                                {mood}
                              </Text>
                              <Text style={styles.moodBreakdownPercentage}>
                                {Math.round(
                                  (count / state.recentMoods.length) * 100
                                )}
                                %
                              </Text>
                            </View>
                ...
                <View style={styles.recentMoods}>
                  {state.recentMoods.slice(0, 8).map((mood, index) => (
                    <TouchableOpacity
                      ...
                      activeOpacity={0.8}
                    >
                      <View style={[styles.recentMoodGradient,{backgroundColor:'#6366f1'}]}>
                        ...
                        <View style={styles.recentMoodRight}>
                          ...
                          <View
                            style={[
                              ...
                              {
                                backgroundColor: getMoodScoreColor(
