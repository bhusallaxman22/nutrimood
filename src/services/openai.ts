...
export const getNutritionSuggestion = async (mood: string, goal: string): Promise<NutritionSuggestion> => {
    ...
    const prompt = `I am feeling ${mood} and my goal is to ${goal}. I know that for my mood, I should eat foods like ${moodInfo.foods.join(', ')} which are rich in ${moodInfo.nutrients.join(', ')}. 
  ...
  "nutrition": {
    ...
    "mainNutrients": ["nutrient1", "nutrient2", "nutrient3"]
        ...
        try {
            ...
            console.error('Error parsing JSON response:', parseError, 'Original text:', suggestionText);
...
const generateFallbackSuggestion = (mood: string, goal: string, moodInfo: any): NutritionSuggestion => {
    ...
    return {
        title: `Perfect ${mealType.charAt(0).toUpperCase() + mealType.slice(1)} for Your ${mood.charAt(0).toUpperCase() + mood.slice(1)} Mood`,
        emoji: "🌟",
        meal: {
        ...
        timing: {
            when: `Best enjoyed ${timeOfDay < 12 ? 'in the morning' : timeOfDay < 17 ? 'in the afternoon' : 'in the evening'}`,
